...
class CUB2011(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        super(CUB2011, self).__init__(root, transform=transform,
    ...
    def _load_metadata(self):
        ...
        data = image.merge(image_class_labels, on = 'img_id')
        self.data = data.merge(train_test_split, on = 'img_id')

        class_names = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'classes.txt'), sep = ' ', names=['class_id'], usecols=[1])
    ...
    def _download(self):
        ...
        with tarfile.open(os.path.join(self.root, self.filename), "r:gz") as f:
            f.extractall(path=os.path.join(self.root))
